...
ByEAR::commit_rw(TxThread* tx)
{
    ...
    if (!sync_bcas(&tx->alive, TX_ACTIVE, TX_COMMITTED))
        tx->tmabort(tx);
    ...
    if (uint32_t owner = lock->owner) {
        switch (threads[owner-1]->alive) {
          ...
          case TX_ACTIVE:
            ...
            if (!sync_bcas(&threads[owner-1]->alive, TX_ACTIVE, TX_ABORTED))
                tx->tmabort(tx);
            break;
          case TX_ABORTED:
...
ByEAR::write_ro(STM_WRITE_SIG(tx,addr,val,mask))
{
    ...
    while (true) {
        ...
        if (uint32_t owner = lock->owner)
            sync_cas(&threads[owner-1]->alive, TX_ACTIVE, TX_ABORTED);
    ...
    for (int i = 0; i < 60; ++i)
        if (lock->reader[i] != 0 && threads[i]->alive == TX_ACTIVE)
            if (!sync_bcas(&threads[i]->alive, TX_ACTIVE, TX_ABORTED))
                tx->tmabort(tx);
