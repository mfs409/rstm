...
OrecFair::commit_rw(TxThread* tx)
{
    ...
    unsigned end_time = 1 + sync_fai(&timestamp.val);
...
OrecFair::validate_committime(TxThread* tx)
{
    if (tx->prio) {
        ...
        while (i != e) {
            ...
            if (ivt.fields.lock && (ivt.all != tx->my_lock)) {
        ...
        foreach (OrecList, i, tx->r_orecs) {
            ...
            ivt.all = (*i)->v.all;
            // if unlocked and newer than start time, abort
            if ((ivt.all > tx->start_time) && (ivt.all != tx->my_lock))
                tx->tmabort(tx);
        }
    }
}
