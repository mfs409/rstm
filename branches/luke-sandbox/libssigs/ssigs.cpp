...
#include "common/platform.hpp"          // stm::sync_bcas
...
static VersionedRecord ssigs[NSIGS] = {};
...
struct WriteLock {
    WriteLock(int sig) : sig_(sig), orig_() {
      ...
      acquire:
        ...
        if (sync_bcas(&ssigs[sig].version, v, v + 1))
...
{
    if (sig < 1 || sig >= NSIGS) {
        fprintf(stderr, "Signal id is out of range: %i", sig);
        _Exit(-1);
    }
...
signal(int sig, sighandler_t handler)
{
    ...
    // we're not currently shadowing the signal so defer to the system
    return (w.isShadowed()) ?
        w.onSignal(sig, handler) :
        call_signal(sig, handler);
}
...
sigaction(int sig, const struct sigaction* act, struct sigaction* out)
{
    ...
    return (w.isShadowed()) ?
        w.onSigaction(sig, act, out) :
        call_sigaction(sig, act, out);
}
