  ...
  {TX_GET_TX_INTERNAL;
      ...
      if (cpending.val + 1 == started.val) {
          ...
          if (cpending.val + 1 == started.val) {
              ...
              *addr = val;
              // go turbo mode
              GoTurbo(tx, CohortsENReadTurbo, CohortsENWriteTurbo, CohortsENCommitTurbo);
              return;
          }
          // reset flag
          inplace.val = 0;
  ...
  CohortsENWriteRW(TX_FIRST_PARAMETER STM_WRITE_SIG(addr,val,mask))
  {
      ...
      if (tx->writes.size(TX_LONE_PARAMETER) >= TIMES && cpending.val + 1 == started.val) {
          ...
          if (cpending.val + 1 == started.val) {
              ...
              *addr = val;
              // go turbo mode
              GoTurbo(tx, ReadTurbo, WriteTurbo, CommitTurbo);
              return;
          }
          // reset flag
          inplace.val = 0;
...
DECLARE_SIMPLE_METHODS_FROM_TURBO(CohortsEN)
REGISTER_FGADAPT_ALG(CohortsEN, "CohortsEN", true)

#ifdef STM_ONESHOT_ALG_CohortsEN
DECLARE_AS_ONESHOT(CohortsEN)
#endif
