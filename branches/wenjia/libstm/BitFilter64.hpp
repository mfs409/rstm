...
#include <stdint.h>
#include "../include/abstract_compiler.hpp"    // TM_ALIGN
...
namespace stm
{
  ...
  class BitFilter64
  {
    ...
    public:
      ...
      /*** simple union */
      inline void unionwith(const BitFilter64& rhs)
      {
      ...
      inline void fastcopy(const volatile BitFilter64* rhs) volatile
      {
          word_filter[0] = rhs->word_filter[0];
          word_filter[1] = rhs->word_filter[1];
      }
      ...
      inline bool intersect(const BitFilter64* rhs) const volatile
      {
          uint32_t t1, t2;
