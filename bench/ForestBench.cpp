    ...
    TM_BEGIN(atomic) {
        if (CFG.running) {
            ...
            for (uint32_t i = 0; i < SET->trees_per_tx; ++i) {
                ...
                // do a lookup?
                if (act < SET->roratio)
                    SET->trees[tree_idx]->lookup(TX_FIRST_ARG val);
                else if (act < SET->insratio)
                    SET->trees[tree_idx]->insert(TX_FIRST_ARG val);
                else
                    SET->trees[tree_idx]->remove(TX_FIRST_ARG val);
            }
        }
    } TM_END;
    *seed = local_seed;
