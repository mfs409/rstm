...
namespace
{
  ...
  OrecELA_amd64_Generic<CM>::Initialize(int id, const char* name)
  {
      ...
      stm::stms[id].commit    = OrecELA_amd64_Generic<CM>::CommitRO;
      stm::stms[id].read      = OrecELA_amd64_Generic<CM>::ReadRO;
      stm::stms[id].write     = OrecELA_amd64_Generic<CM>::WriteRO;
  ...
  template <class CM>
  void
  OrecELA_amd64_Generic<CM>::CommitRO(TX_LONE_PARAMETER)
  {
      TX_GET_TX_INTERNAL;
      // notify CM
      CM::onCommit(tx);
  ...
  template <class CM>
  void
  OrecELA_amd64_Generic<CM>::CommitRW(TX_LONE_PARAMETER)
  {
      TX_GET_TX_INTERNAL;
      // acquire locks
   ...
   */
  template <class CM>
  void* OrecELA_amd64_Generic<CM>::ReadRO(TX_FIRST_PARAMETER STM_READ_SIG(addr,))
  {
      TX_GET_TX_INTERNAL;
      // get the orec addr
  ...
  template <class CM>
  void*
  OrecELA_amd64_Generic<CM>::ReadRW(TX_FIRST_PARAMETER STM_READ_SIG(addr,mask))
  {
      TX_GET_TX_INTERNAL;
  ...
  template <class CM>
  void
  OrecELA_amd64_Generic<CM>::WriteRO(TX_FIRST_PARAMETER STM_WRITE_SIG(addr,val,mask))
  {
      TX_GET_TX_INTERNAL;
      // add to redo log
  ...
  template <class CM>
  void
  OrecELA_amd64_Generic<CM>::WriteRW(TX_FIRST_PARAMETER STM_WRITE_SIG(addr,val,mask))
  {
      TX_GET_TX_INTERNAL;
      // add to redo log
      tx->writes.insert(WriteSetEntry(STM_WRITE_SET_ENTRY(addr, val, mask)));
