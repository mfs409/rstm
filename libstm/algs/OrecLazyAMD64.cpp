...
namespace stm
{
  ...
  TM_FASTCALL void* OrecLazyAMD64ReadRO(TX_FIRST_PARAMETER STM_READ_SIG(,));
  TM_FASTCALL void* OrecLazyAMD64ReadRW(TX_FIRST_PARAMETER STM_READ_SIG(,));
  TM_FASTCALL void OrecLazyAMD64WriteRO(TX_FIRST_PARAMETER STM_WRITE_SIG(,,));
  TM_FASTCALL void OrecLazyAMD64WriteRW(TX_FIRST_PARAMETER STM_WRITE_SIG(,,));
  TM_FASTCALL void OrecLazyAMD64CommitRO(TX_LONE_PARAMETER);
  TM_FASTCALL void OrecLazyAMD64CommitRW(TX_LONE_PARAMETER);
  NOINLINE void OrecLazyAMD64Validate(TxThread*);
   ...
   */
  void OrecLazyAMD64Begin(TX_LONE_PARAMETER)
  {
      TX_GET_TX_INTERNAL;
  ...
  /**
   *  OrecLazyAMD64 commit (read-only context)
   *
   *    We just reset local fields and we're done
   */
  void
  OrecLazyAMD64CommitRO(TX_LONE_PARAMETER)
  {
      TX_GET_TX_INTERNAL;
      // read-only
      tx->r_orecs.reset();
  ...
  OrecLazyAMD64CommitRW(TX_LONE_PARAMETER)
  {
      ...
      tx->locks.reset();
      OnRWCommit(tx);
      ResetToRO(tx, OrecLazyAMD64ReadRO, OrecLazyAMD64WriteRO, OrecLazyAMD64CommitRO);
  }

  /**
   *  OrecLazyAMD64 read (read-only context):
   *
   *    in the best case, we just read the value, check the timestamp, log the
   *    orec and return
   */
  void* OrecLazyAMD64ReadRO(TX_FIRST_PARAMETER STM_READ_SIG(addr,))
  {
      TX_GET_TX_INTERNAL;
      // get the orec addr
  ...
  /**
   *  OrecLazyAMD64 read (writing context):
   *
   *    Just like read-only context, but must check the write set first
   */
  void*
  OrecLazyAMD64ReadRW(TX_FIRST_PARAMETER STM_READ_SIG(addr,mask))
  {
      TX_GET_TX_INTERNAL;
      ...
      void* val = OrecLazyAMD64ReadRO(TX_FIRST_ARG addr STM_MASK(mask));
      REDO_RAW_CLEANUP(val, found, log, mask);
      return val;
  }

  /**
   *  OrecLazyAMD64 write (read-only context):
   *
   *    Buffer the write, and switch to a writing context
   */
  void
  OrecLazyAMD64WriteRO(TX_FIRST_PARAMETER STM_WRITE_SIG(addr,val,mask))
  {
      TX_GET_TX_INTERNAL;
      // add to redo log
      tx->writes.insert(WriteSetEntry(STM_WRITE_SET_ENTRY(addr, val, mask)));
      OnFirstWrite(tx, OrecLazyAMD64ReadRW, OrecLazyAMD64WriteRW, OrecLazyAMD64CommitRW);
  }

  /**
   *  OrecLazyAMD64 write (writing context):
   *
   *    Just buffer the write
   */
  void
  OrecLazyAMD64WriteRW(TX_FIRST_PARAMETER STM_WRITE_SIG(addr,val,mask))
  {
      TX_GET_TX_INTERNAL;
      // add to redo log
      tx->writes.insert(WriteSetEntry(STM_WRITE_SET_ENTRY(addr, val, mask)));
  ...
  /**
   *  OrecLazyAMD64 in-flight irrevocability:
   *
   *    Either commit the transaction or return false.
   */
   bool OrecLazyAMD64Irrevoc(TxThread*)
   {
       return false;
       // NB: In a prior release, we actually had a full OrecLazyAMD64 commit
   ...
   *    inline it.
   */
  void OrecLazyAMD64Validate(TxThread* tx)
  {
      foreach (OrecList, i, tx->r_orecs)
  ...
  /**
   *  Switch to OrecLazyAMD64:
   *
   *    The timestamp must be >= the maximum value of any orec.  Some algs use
   *    timestamp as a zero-one mutex.  If they do, then they back up the
   *    timestamp first, in timestamp_max.
   */
  void OrecLazyAMD64OnSwitchTo()
  {
...
DECLARE_AS_ONESHOT(OrecLazyAMD64)
#endif
