 ...
 */
template scope_t* oreceager_generic::rollback_generic<BackoffCM>(TX*);
/**
 * Instantiate tm_begin with the appropriate CM for this TM algorithm
 */
template void oreceager_generic::tm_begin_generic<BackoffCM>(scope_t*);
/**
 * Instantiate tm_end with the appropriate CM for this TM algorithm
 */
template void oreceager_generic::tm_end_generic<BackoffCM>();

namespace oreceagerbackoff
{
  /**
   * Create aliases to the oreceager_generic functions or instantiations that
   * we shall use for oreceager
   */
  scope_t* rollback(TX* tx) __attribute__((weak, alias("_ZN17oreceager_generic16rollback_genericIN3stm9BackoffCMEEEPvPNS1_2TXE")));

  void tm_begin(scope_t *) __attribute__((weak, alias("_ZN17oreceager_generic16tm_begin_genericIN3stm9BackoffCMEEEvPv")));
  void tm_end() __attribute__((weak, alias("_ZN17oreceager_generic14tm_end_genericIN3stm9BackoffCMEEEvv")));
  TM_FASTCALL
 ...
 */
REGISTER_TM_FOR_ADAPTIVITY(OrecEagerBackoff, oreceagerbackoff);
REGISTER_TM_FOR_STANDALONE(oreceagerbackoff, 16);
